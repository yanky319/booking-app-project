...
namespace BL
{
    public class BL_imp : IBL
    {
        ...
        {
            int index = getHosts().ToList().FindIndex(e => e.HostID == key);
            if (index == -1)
        ...
        public IEnumerable<HostingUnit> GetHostsHostingUnits(Host host, Func<HostingUnit, bool> predicate = null)
        {
            var a = from item in getHostingUnits()
        ...
        public IEnumerable<Order> getHostsOrders(Host host, Func<Order, bool> predicate = null)
        {
            var a = from item in getOrders()
                    where item.HostID == host.HostID
                    select item;
        ...
        public void updateOrder(int key, OrderStatus status)
        {
            ...
            if (status == OrderStatus.mail_sent && !getHost(order.HostID).CollectionClearance)
